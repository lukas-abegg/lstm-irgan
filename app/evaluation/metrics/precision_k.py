...
def measure_precision_at_k_eval_all(x_data_query, y_data_query, pred_scores_query, k, sess):
    p = 0.0
    cnt = 0
    ...
    for query_id in x_data_query.keys():
        ...
        if len(rated_document_scores_order) >= k:
...
def measure_precision_at_k(model, x_val, ratings_data, queries_data, documents_data, k, sess):
    ...
    print("Prediction scores for p_"+str(k)+": ", pred_scores_all, "| found scores =", len(pred_scores_all), " for ", len(eval_queries_all), " queries")
