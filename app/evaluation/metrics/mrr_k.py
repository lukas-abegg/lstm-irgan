...
def measure_mrr_eval_all(x_data_query, y_data_query, pred_scores_query, k, sess):
    rs = []
...
def measure_mrr(model, x_val, ratings_data, queries_data, documents_data, k, sess):
    ...
    query_ids_all, x_data_all, y_data_all, eval_queries_all, eval_documents_all = utils.prepare_eval_data(
        x_val, ratings_data, queries_data, documents_data)

    pred_scores_all = model.get_prob(eval_queries_all, eval_documents_all)
    print("Prediction scores for mrr_"+str(k)+": ", pred_scores_all)
